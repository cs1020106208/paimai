...
<div class="wrap">
  ...
  <form action="${pageContext.request.contextPath}/auction/updateAuction.do" method="post" enctype="multipart/form-data" onsubmit="return form()">
      <div class="login logns produce">
        <h1 class="blues">拍卖品信息</h1>
          ...
          <input type="hidden" name="auction_id" value="${auction.auction_id}">
            ...
            <dd>
              <label>起拍价：</label>
              <input type="text" class="inputh lf" name="auction_start_price" value="${auction.auction_start_price}" id="start_Price" onblur="startPrice()"/>
              <div class="lf red laba" id="startPriceDiv"></div>
            </dd>
            <dd>
              <label>底价：</label>
              <input type="text" class="inputh lf" name="auction_upset" value="${auction.auction_upset}" id="end_Price" onblur="endPrice()" />
              <div class="lf red laba" id="endPriceDiv"></div>
            </dd>
            <dd>
              <label>开始时间：</label>
              <input type="text" class="inputh lf" name="auction_start_time" id="start_Time" onblur="startTime()"  value="<fmt:formatDate value="${auction.auction_start_time}" pattern="yyyy-MM-dd hh:mm:ss"/>" />
              <div class="lf red laba" id="startTimeDiv"></div>
            </dd>
            <dd>
              <label>结束时间：</label>
              <input type="text" class="inputh lf" name="auction_end_time" id="end_Time" onblur="endTime()" value="<fmt:formatDate value="${auction.auction_end_time}" pattern="yyyy-MM-dd hh:mm:ss"/>" />
              <div class="lf red laba" id="endTimeDiv"></div>
            </dd>
            <dd class="dds">
              <label>描述：</label>
              <textarea  cols="" rows="" name="auction_desc" class="textarea">${auction.auction_desc}</textarea>
            </dd>
            <dd>
              <label>修改图片：</label>
